...
class aspect_rs(nn.Module):
    def __init__(self):
        ...
        torch.manual_seed(0); self.embedding_user_free = nn.Embedding(conf.num_users, conf.embedding_dim)
        torch.manual_seed(0); self.embedding_item_free = nn.Embedding(conf.num_items, conf.embedding_dim)
    ...
    def forward(self, historical_review, # (batch_size, sequence_length)
        ...
        for batch_idx, histor_idx_list in user_histor_idx.items():
            ...
            user_aspect_embedding.append(torch.mean(tmp_user_aspect_embedding, dim=0))
        
        for batch_idx, histor_idx_list in item_histor_idx.items():
            tmp_item_aspect_embedding = []
            for idx in histor_idx_list:
                tmp_item_aspect_embedding.append(r_s[idx])
            item_aspect_embedding.append(torch.mean(tmp_item_aspect_embedding, dim=0))
        ...
        user_free_embedding = self.embedding_user_free(user) # (batch_size, mf_dimension)
        item_free_embedding = self.embedding_item_free(item) # (batch_size, mf_dimension)

        # (batch_size, mf_dimension)
        output_emb = user_aspect_embedding * item_aspect_embedding \
        ...
        obj = conf.lr_abae_1 * U_loss + conf.lr_abae_2 * J_loss + conf.lr_pmf * rating_loss

        return prediction.view(-1), obj, rating_loss, abae_loss