...
if __name__ == '__main__':
    ...
    ########################### FIRST TRAINING #####################################
    log = Logging('/content/drive/My Drive/task/aspect_based_rs/out/amazon_electronics/train_amazon_electronics_language_model_id_x.log')
    log_detail = Logging('/content/drive/My Drive/task/aspect_based_rs/out/amazon_electronics/detail_train_amazon_electronics_language_model_id_x.log')
    train_model_path = '/content/drive/My Drive/task/aspect_based_rs/out/amazon_electronics/train_amazon_electronics_language_model_id_x.mod'
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        for batch_idx_list in train_batch_sampler:
            ...
            hidden_state = (torch.zeros(2, review_input.shape[1], conf.hidden_size).cuda(), \
                torch.zeros(2, review_input.shape[1], conf.hidden_size).cuda())
            review_output_embedding, _ = model(review_input, review_output, hidden_state)
            out = model.out(review_output_embedding, review_output.view(-1))
