...
class encoder(nn.Module):
    def __init__(self):
        ...
        self.hidden_layer = nn.Linear(2*conf.mf_dim, conf.hidden_dim)
    ...
    def forward(self, user, item):
        ...
        return hidden_state, mlp_concat_emebd.view(1, user.shape[0], -1)
...
class decoder_rating(nn.Module):
    ...
    def forward(self, mlp_concat_emebd):
        pred = self.final_linear(final_embed).view(-1) + conf.avg_rating
