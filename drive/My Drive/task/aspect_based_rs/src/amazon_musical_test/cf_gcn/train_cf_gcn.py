...
if __name__ == '__main__':
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        for batch_idx_list in train_batch_sampler:
            user, item, label, review_input, review_target, user_doc, item_doc =\
                train_dataset.get_batch(batch_idx_list)
            review_out_loss, rating_out_loss, obj = model(user, item, label, \
                review_input, review_target, user_doc, item_doc)

            #review_out_loss, obj = model(user, item, \
            #    review_input, review_target, user_doc, item_doc)
        ...
        if epoch % 5 == 0:
            val_bleu_4, val_rouge_L_f = evaluate(review_val_dataset, \
                review_val_sampler, model, user_doc, item_doc)
            ...
            test_bleu_4, test_rouge_L_f = evaluate(review_test_dataset, \
                review_test_sampler, model, user_doc, item_doc)
