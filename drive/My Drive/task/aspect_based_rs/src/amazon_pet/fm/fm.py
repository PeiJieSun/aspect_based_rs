...
class decoder_fm(nn.Module):
    ...
    def reinit(self):
        torch.manual_seed(0); nn.init.xavier_uniform_(self.free_user_embedding.weight)
        torch.manual_seed(0); nn.init.xavier_uniform_(self.free_item_embedding.weight)
    ...
    def forward(self, user, item):
        ...
        fm_interactions_2 = torch.mm(torch.pow(input_vec, 2),
                                     torch.pow(self.fm_V, 2))
        fm_output = 0.5 * torch.sum(fm_interactions_1 - fm_interactions_2, 1, keepdim=True)\
            + fm_linear_part + 0*self.b_users[user] + 0*self.b_items[item]  + conf.avg_rating
