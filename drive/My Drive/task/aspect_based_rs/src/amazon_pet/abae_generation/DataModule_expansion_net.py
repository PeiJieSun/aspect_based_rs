...
def load_all():
    ...
    for idx, line in enumerate(f):
        line = eval(line)
        user, item, rating, g_review, abae_review = \
            line['user'], line['item'], line['rating'], line['g_review'], line['abae_review']
        review_in, review_out = generate_review(g_review)

        g_review = g_review[:conf.rev_len]
        g_review.extend([PAD]*(conf.rev_len-len(g_review)))
        train_data[idx] = [user, item, rating, review_in, review_out, g_review, abae_review]
...
class TrainData():
    ...
    def get_batch(self, batch_idx_list):
        ...
        for data_idx in batch_idx_list:
            ...
            ####### prepare for abae extraction #######
            user, item, label = self.train_data[data_idx][0], \
                self.train_data[data_idx][1], self.train_data[data_idx][2]
...
class TestData():
    def __init__(self, test_data, user_seq_dict=None, item_seq_dict=None):
    ...
    def get_batch(self, batch_idx_list):
        ...
        for data_idx in batch_idx_list:
            ...
            real_review_list.append(self.test_data[data_idx][5]) #(batch_size, seq_length)


            ####### prepare for abae extraction #######
            user, item, label = self.test_data[data_idx][0], \
                self.test_data[data_idx][1], self.test_data[data_idx][2]
