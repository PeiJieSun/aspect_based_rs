...
import DataModule_aspect_4 as data_utils
...
if __name__ == '__main__':
    ...
    log = Logging('%s/train_%s_aspect_rating_1_id_X.py' % (conf.out_path, conf.data_name))
    train_model_path = '%s/train_%s_aspect_rating_1_id_X.mod' % (conf.out_path, conf.data_name)
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        for batch_idx_list in train_batch_sampler:
            user_list, item_list, rating_list, pos_review, \
                neg_review, user_idx_list, item_idx_list = train_dataset.get_batch(batch_idx_list)

            obj, rating_loss, abae_loss, prediction = model(pos_review, neg_review, \
