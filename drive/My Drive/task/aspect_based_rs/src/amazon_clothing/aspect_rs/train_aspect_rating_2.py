...
if __name__ == '__main__':
    ...
    train_data, val_data, test_data, train_user_historical_review_dict, \
        train_item_historical_review_dict, user_doc_dict, item_doc_dict = data_utils.load_all()
    t1 = time()
    print('Data has been loaded successfully, cost:%.4fs' % (t1 - t0))

    ########################### FIRST TRAINING #####################################
    check_dir('%s/train_%s_aspect_rating_1_id_x.log' % (conf.out_path, conf.data_name))
    log = Logging('%s/train_%s_aspect_rating_1_id_50.py' % (conf.out_path, conf.data_name))
    train_model_path = '%s/train_%s_aspect_rating_1_id_50.mod' % (conf.out_path, conf.data_name)

    # prepare data for the training stage
    train_dataset = data_utils.TrainData(train_data, \
        train_user_historical_review_dict, train_item_historical_review_dict, \
        train_data, user_doc_dict, item_doc_dict)
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        for batch_idx_list in train_batch_sampler:
            user_list, item_list, rating_list, review_input_list, \
                ...
                item_histor_index, item_histor_value, user_doc, item_doc = \
                train_dataset.get_batch(batch_idx_list)

            obj, rating_loss, abae_loss, prediction = model(review_input_list, \
                neg_review, user_list, item_list, rating_list, user_histor_index, \
                user_histor_value, item_histor_index, item_histor_value, user_doc, item_doc)
