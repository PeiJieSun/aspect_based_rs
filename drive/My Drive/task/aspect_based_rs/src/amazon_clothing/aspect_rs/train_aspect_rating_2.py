...
import DataModule_aspect_2 as data_utils
...
if __name__ == '__main__':
    ...
    ############################## CREATE MODEL ##############################
    from aspect_rating_2 import aspect_rating_1
    ...
    for idx in range(3):
        model_params['user_word_embedding.weight'][idx] = torch.zeros(conf.word_dimension)
        model_params['item_word_embedding.weight'][idx] = torch.zeros(conf.word_dimension)
    for idx in range(3, conf.vocab_sz):
        model_params['user_word_embedding.weight'][idx] = torch.FloatTensor(word_embedding.wv[word_embedding.wv.index2entity[idx-3]])
        model_params['item_word_embedding.weight'][idx] = torch.FloatTensor(word_embedding.wv[word_embedding.wv.index2entity[idx-3]])

    k_means_weight = np.load('%s/%s.k_means.npy' % (conf.target_path, conf.data_name))
    model_params['user_transform_T.weight'] = torch.FloatTensor(k_means_weight.transpose()) # (aspect_dimesion, word_dimension)
    model_params['item_transform_T.weight'] = torch.FloatTensor(k_means_weight.transpose()) # (aspect_dimesion, word_dimension)

    model.load_state_dict(model_params)
    
    #model.load_state_dict(torch.load('/content/drive/My Drive/task/aspect_based_rs/out/model/train_amazon_clothing_aspect_rating_1_id_adabound_32.mod'))
    ...
    log = Logging('%s/train_%s_aspect_rating_1_id_adabound_33.py' % (conf.out_path, conf.data_name))
    train_model_path = '%s/train_%s_aspect_rating_1_id_adabound_33.mod' % (conf.out_path, conf.data_name)
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        train_rating_loss, train_u_abae_loss, train_i_abae_loss, train_prediction = [], [], [], []
        
        for batch_idx_list in train_batch_sampler:
            user_list, item_list, rating_list, user_review_input_list, user_neg_review, \
                item_review_input_list, item_neg_review, user_histor_index, user_histor_value, \
                item_histor_index, item_histor_value = train_dataset.get_batch(batch_idx_list)

            obj, rating_loss, u_abae_loss, i_abae_loss, prediction, user_aspect_embed, item_aspect_embed = \
                model(user_review_input_list, user_neg_review, item_review_input_list, item_neg_review, \
                user_list, item_list, rating_list, user_histor_index, user_histor_value, item_histor_index, item_histor_value)

            train_rating_loss.extend(tensorToScalar(rating_loss)); train_u_abae_loss.extend(tensorToScalar(u_abae_loss))
            train_i_abae_loss.extend(tensorToScalar(i_abae_loss)); train_prediction.extend(tensorToScalar(prediction))
        ...
        log.record('ABAE: Train User loss:{:.4f}, Item loss:{:.4f}'.format(np.mean(train_u_abae_loss), np.mean(train_i_abae_loss)))
