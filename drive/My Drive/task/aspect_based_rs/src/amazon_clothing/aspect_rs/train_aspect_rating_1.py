...
if __name__ == '__main__':
    ...
    log = Logging('%s/train_%s_aspect_rating_1_id_adabound_12.log' % (conf.out_path, conf.data_name))
    train_model_path = '%s/train_%s_aspect_rating_1_id_adabound_12.mod' % (conf.out_path, conf.data_name)
    ...
    for epoch in range(1, conf.train_epochs+1):
        ...
        for batch_idx_list in val_batch_sampler:
            ...
            prediction, mse_loss = model.predict(user_list, item_list, rating_list)
            val_prediction.extend(tensorToScalar(prediction)); val_rating_loss.extend(tensorToScalar(mse_loss))
        t2 = time()
        ...
        for batch_idx_list in test_batch_sampler:
            ...
            prediction, mse_loss = model.predict(user_list, item_list, rating_list)
            test_prediction.extend(tensorToScalar(prediction)); test_rating_loss.extend(tensorToScalar(mse_loss))
        t3 = time()
